...
### 2-4. 상수의 위치
지난 1주차 과제를 할 때 상수를 서로 import 하고 사용하는 부분이 과연 좋을까를 고민했었다. 1주차에서는 그래서 상수를 **독립적인 constant**디렉토리에 모아두고 필요한 파일에서 import를 해서 사용했었다.

그런데 이번 2주차 과제를 하면서는 생각이 조금 바뀐 것 같다. 그래서 이번에는 관련성이 가장 깊은 파일에 상수를 위치시켜봤다. 그런데 이렇게 했을 때 이점은 해당 파일 안에서만 사용되는 상수를 굳이 다른 상수 파일에 독립적으로 위치시켜서 export를 하지 않아도 된다는 것이다. 그리고 같은 파일에 위치하는 것이 파일 내에서만 사용되는 상수라는 것을 더 잘 표현하는 것 같았다.

View 클래스가 있는 파일에 선언된 상수를 다른 파일에서 사용해야 하는 경우가 있었는데 이 때는 View 클래스가 있는 파일에서 import를 해서 사용했다. 이렇게 하는 것이 두 파일이 서로 같은 상수를 공유하고 연관이 있다는 것을 의도를 잘 담은 방법이라고 생각했다.

저번 주차와는 생각이 바뀌어서 상수의 위치를 지난 주차와는 다르게 구현해봤다. 상황에 따라 다르다고 생각하지만 어떻게 하는 방식이 가장 좋을지는 아직도 잘 모르겠고 여러 선배 개발자들의 경험을 통한 조언이 필요한 부분이기도 하다.
