...
그런데 처음에 이러한 설계를 하고 기능을 구현하는데 커피 제조하는 도메인의 예시처럼 명확하게 설명이 가능해야 한다고 생각하고 해보니까 막상 클래스를 나누고 메시지들을 실행하는 것이 상당히 어려웠다. 그래서 Car 클래스와 Input 클래스를 제외하고는 원래 개발하듯이 함수위주로 기능을 구현하게 됐다.
